...
Update: These issues were fixed in subsequent commits (e.g., `c3aa5ad`, `b693c81`), which switched tool responses to `text` content with `structuredContent`, added `isError` for tool failures, and removed non-spec `requestId` fields from responses. Codex CLI and the MCP plugin now work with the server.
